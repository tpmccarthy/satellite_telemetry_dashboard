...
async def test_post_telemetry_constraints(transport):
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        payload = {"satelliteId": "SAT-1", "timestamp": "2026-02-23T12:00:00Z", "altitude": 400.5, "velocity": 7.6, "status": "healthy"}
...
async def test_get_telemetry_filtering(transport):
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        await ac.post("/telemetry", json={"satelliteId": "V-1", "timestamp": "2026-02-23T12:00:00Z", "altitude": 100, "velocity": 10, "status": "healthy"})
...
async def test_delete_telemetry_lifecycle(transport):
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        # 1. Create
        post_res = await ac.post("/telemetry", json={"satelliteId": "DEL-1", "timestamp": "2026-02-23T12:00:00Z", "altitude": 100, "velocity": 10, "status": "healthy"})
